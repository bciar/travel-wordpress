...
class Transfers_Plugin_Post_Types extends Transfers_BaseSingleton {
	...
	function get_availability_entry($entry_id, $duration_minutes = 0, $given_date = null) {
		...
		$sql .= "		
			...
			transport_types.post_title as transport_type,transport_types.ID as transport_post_ID
		FROM " . TRANSFERS_AVAILABILITY_TABLE . " availability ";
				
		$sql .= " INNER JOIN $wpdb->posts destinations1 ON ";
		$sql .= " (destinations1.ID = availability.destination_from_id OR availability.destination_from_id = 0)";
		
		$sql .= " INNER JOIN $wpdb->posts destinations2 ON ";
		$sql .= " (destinations2.ID = availability.destination_to_id OR availability.destination_to_id = 0) ";
	...
	function list_connecting_destinations($orderby = 'wp_posts.post_title', $order = 'ASC', $destination_from_id) {
		...
		//add_filter('posts_join', array($this, 'connecting_destinations_join'), 10, 2 );
		...
		remove_filter('posts_join', array($this, 'connecting_destinations_join') );
	...
	function connecting_destinations_join($join, &$wp_query) {
		...
		if ( isset($wp_query->query_vars['post_type']) && $wp_query->query_vars['post_type'] == 'destination' ) {
			...
			if (isset($wp_query->query_vars['tf_destination_from_id'])) {
				...
				$join .= $wpdb->prepare(" INNER JOIN " . $wpdb->prefix . "posts posts_from ON posts_from.post_status='publish' AND posts_from.ID = %d ", $destination_from_id);
			}
		}
	...
	function connecting_destinations_where( $where, &$wp_query ) {
		...
		if ( isset($wp_query->query_vars['post_type']) && $wp_query->query_vars['post_type'] == 'destination' ) {
			...
			if (isset($wp_query->query_vars['tf_destination_from_id'])) {
				...
				// $where .= " AND $wpdb->posts.post_parent = posts_from.post_parent ";
			}
		}
	...
	function get_booking_entry($entry_id) {
		...
		$sql .= " LEFT JOIN $wpdb->posts destinations4 ON ";
		$sql .= " destinations4.ID = bookings.from_id ";

		$sql .= " LEFT JOIN $wpdb->posts destinations5 ON ";
		$sql .= " destinations5.ID = bookings.to_id ";
	...
	function create_booking_entry($booking_args) {
		...
		$wpdb->query($wpdb->prepare($sql, $user_id, $from_id, $to_id, $first_name, $last_name, $email, $phone, $address, $address_2, $company, $town, $zip, $state, $country, serialize($booking_object->other_fields), $availability_id, $people_count, $is_private, $total_price, $booking_datetime));	
